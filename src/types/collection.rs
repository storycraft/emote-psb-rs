...
use super::{number::PsbNumber, PsbValue, PSB_TYPE_INTEGER_ARRAY_N};
...
impl PsbUintArray {
    ...
    pub fn get_item_n(&self) -> u8 {
        PsbNumber::get_uint_n(*self.vec.iter().max().unwrap_or(&0))
    }

    pub fn get_n(&self) -> u8 {
        PsbNumber::get_uint_n(self.vec.len() as u64)
    }
